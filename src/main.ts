...
class InboxProcessorPlugin extends Plugin {
    ...
    onunload() {
        ...
        clearInterval(this.interval); // Clear the interval when the plugin is unloaded
    }
                    ...
                    if (file instanceof TFile) {
                        for (const rule of this.settings.rules) {
                            ...
                            if (extensionsRegex.test(file.name) && (!nameRegex || nameRegex.test(file.name.replace(/\.[^/.]+$/, '')))) {
                                ...
                                if (this.settings.convertExtensionsToLowercase) {
                                    const newFileName = file.name.replace(/\.[^/.]+$/, ext => ext.toLowerCase());
                                    if (newFileName !== file.name) {
                                        const newPath = `${file.parent.path}/${newFileName}`;
                                        await this.app.vault.rename(file, newPath);
                                        file = this.app.vault.getAbstractFileByPath(newPath) as TFile; // Update the file reference
                                    }
                                }
    ...
    async moveFile(file: TFile, rootFolder: string, folderStructure: string, regex: string) {
        if (!folderStructure) {
    ...
    extractDateFromFileName(fileName, regex) {
